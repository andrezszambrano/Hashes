...
/*
void imprimir_listas(void* elemento, void* inservible){

  elemento_lista_t* ptr=(elemento_lista_t*)elemento;

  printf("CLAVE: %s, ELEMENTO: %s. \n", ptr->clave, ((char*)(ptr->elemento)));


}


void recorrer_hash(hash_t* hash){

  int contador=0;
  while(contador<hash->capacidad){
    if(hash->vector_listas[contador]){
      printf("*");
      lista_con_cada_elemento(hash->vector_listas[contador], &imprimir_listas, NULL);
    }
    else
      printf("POSICION VACÍA\n");
